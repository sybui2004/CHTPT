...
const DEFAULT_MONGO_URI = 'mongodb://root:root@localhost:27018/product_db?directConnection=true&authSource=admin';
const MONGO_URI = process.env.MONGO_URI || DEFAULT_MONGO_URI;

// Separate database URIs for proper data separation
const AUTH_DB_URI = 'mongodb://root:root@mongo:27017/auth_db?directConnection=true&authSource=admin';
const SHOP_DB_URI = 'mongodb://root:root@mongo:27017/shop_db?directConnection=true&authSource=admin';
const PRODUCT_DB_URI = 'mongodb://root:root@mongo:27017/product_db?directConnection=true&authSource=admin';
...
async function getOrCreateRole(db) {
    let role = await db.collection('roles').findOne({ name: 'USER' });
    if (!role) {
        role = {
            _id: objectId(),
            name: 'USER',
...
async function getOrCreateUser(db, role, seedShop) {
...
async function getOrCreateShop(db, user, seedShop) {
...
async function getOrCreateCategories(db) {
    const root = await upsertCategory(db, {
        name: 'Danh muc demo',
    ...
    for (const name of names) {
        categories[name] = await upsertCategory(db, {
            name,
...
async function updateCounters(db, shop, categories) {
    const productCategoryIds = await db.collection('products')
        .distinct('category', { shop: shop._id, visible: true, deleted: false });
    const shopProductCount = await db.collection('products').countDocuments({
        shop: shop._id,
        visible: true,
        deleted: false
    });
    ...
    await db.collection('shop_categories').updateOne(
...
async function seed() {
    ...
    try {
        ...
        for (const [shopIndex, seedShop] of demoShops.entries()) {
            ...
            const products = await createProducts(productDb, shop, seedShop, categories, shopIndex);
            await updateCountersInDb(productDb, shop, categories);
...
async function updateCountersInDb(db, shop, categories) {
    const productCategoryIds = await db.collection('products')
        .distinct('category', { shop: shop._id, visible: true, deleted: false });
    const shopProductCount = await db.collection('products').countDocuments({
        shop: shop._id,
